...
### 配置初始状态
要运行初链私有链，需要定义自己的创世区块，创世区块信息写在一个 JSON 格式的配置文件中。

首先将下面的内容保存到一个 JSON 文件中，例如 genesis.json。

用户可选择单委员会或者多委员会运行程序。多委员会情况下，委员会节点要都启动起来，程序才能正常运转。
...
其中，chainID 指定了独立的区块链网络 ID。

网络 ID 在连接到其他节点的时候会用到，初链公网的网络 ID 是 1，为了不与公有链网络冲突，运行私有链节点的时候要指定自己的网络 ID。

不同 ID 网络的节点无法相互连接。”alloc”默认的初始账户，用户可以自定义。"committee"委员会配置账户地址和公钥。

配置文件还对当前挖矿难度 difficulty、区块 Gas 消耗限制 gasLimit 等参数进行了设置。
